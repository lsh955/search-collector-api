...
exports.get = async (value) => {
	...
	const data = await htmlParsing(`https://openapi.naver.com/v1/search/kin?query=${encodeURI(PARAMS.query)}&start=${PARAMS.start}&display=${PARAMS.display}&sort=${PARAMS.sort}`);
...
async function searchList(value) {
	...
	for (const [index, data] of Object.entries(JSON.parse(value)['items'])) {
		...
		const description = _.replace(data.description, /(<([^>]+)>)/g, ''); // 질문내용
