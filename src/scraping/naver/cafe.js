...
exports.get = async (value) => {
	...
	const data     = await htmlParsing(`https://openapi.naver.com/v1/search/cafearticle?query=${encodeURI(PARAMS.query)}&start=${PARAMS.start}&display=${PARAMS.display}&sort=${PARAMS.sort}`);
...
async function searchList(value) {
	...
	for (const [index, data] of Object.entries(JSON.parse(value)['items'])) {
		...
		const uri         = _.replace(data.link, /(<([^>]+)>)/g, '');	// 포스팅 URI
		const description = _.replace(data.description, /(<([^>]+)>)/g, '');	// 내용
		const cafeName    = _.replace(data.cafename, /(<([^>]+)>)/g, '');	// 카페 이름(<title></title>)
		const cafeUrl     = _.replace(data.cafeurl, /(<([^>]+)>)/g, ''); // 카페 URI

		results.push({
			title      : title === undefined ? null : title,
			uri        : uri === undefined ? null : uri,
			description: description === undefined ? null : description,
			cafeName   : cafeName === undefined ? null : cafeName,
			cafeUrl    : cafeUrl === undefined ? null : cafeUrl
		});
	}
...
async function scanFilter(value) {
	...
	let $ = cheerio.load(value.substring(value.indexOf('<body>'), value.indexOf('</body>')))


	// TODO :: 여기서 네이버 카페 작업시작....


	// 스캔키워드
	let keyword = (PARAMS['scan'] || '').split(',');
