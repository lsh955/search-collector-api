...
exports.get = async (value) => {
	...
	const data = await htmlParsing(`https://dapi.kakao.com/v2/search/blog?query=${encodeURI(PARAMS.query)}&page=${PARAMS.page}&size=${PARAMS.size}&sort=${PARAMS.sort}`);
...
async function searchList(value) {
	...
	for (const [index, data] of Object.entries(JSON.parse(value)['documents'])) {
		...
		const blogName    = _.replace(data.blogname, /(<([^>]+)>)/g, '');	// 블로그 이름(<title></title>)
		const description = _.replace(data.contents, /(<([^>]+)>)/g, '');  // 포스팅 내용
		const postUri     = _.replace(data.url, /(<([^>]+)>)/g, ''); 	// 블로그 URI
