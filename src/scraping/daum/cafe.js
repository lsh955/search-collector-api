...
exports.get = async (value) => {
	...
	const data = await htmlParsing(`https://dapi.kakao.com/v2/search/cafe?query=${encodeURI(PARAMS.query)}&page=${PARAMS.page}&size=${PARAMS.size}&sort=${PARAMS.sort}`);
...
async function searchList(value) {
	...
	for (const [index, data] of Object.entries(JSON.parse(value)['documents'])) {
		...
		const uri         = _.replace(data.url, /(<([^>]+)>)/g, '') + `?q=${encodeURI(PARAMS.query)}`;	// URI

		results.push({
			title      : title === undefined ? null : title,
			name       : cafeName === undefined ? null : cafeName,
...
async function scanFilter(value) {
	...
	// 네이버 블로그 iframe 경우
	if (value.search(/#down/g) > -1)
		value = await htmlParsing('https://cafe.daum.net' + $('#down').attr('src'));
